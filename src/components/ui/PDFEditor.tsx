...
export default function PDFEditor() {
  ...
  useEffect(() => {
    if (iframeRef.current) {
      iframeRef.current.src = `data:application/pdf;base64,${pdfBase64}#page=${currentPage + 1}`
    }
    if (pdfBase64) {
  ...
  const handleAddPage = async () => {
    if (pdfBase64) {
      try {
        ...
        const pdfDoc = await PDFDocument.load(atob(pdfBase64))
        
        // Add a new page
        const newPage = pdfDoc.insertPage(currentPage + 1)
        
        // If there's a previous page, match its size
        if (currentPage >= 0) {
          const prevPage = pdfDoc.getPage(currentPage)
          newPage.setSize(prevPage.getWidth(), prevPage.getHeight())
        }
        
        // Save the modified PDF
        const pdfBytes = await pdfDoc.save()
        const newPdfBase64 = btoa(String.fromCharCode.apply(null, new Uint8Array(pdfBytes) as unknown as number[]))
        
        // Update state
        setPdfBase64(newPdfBase64)
